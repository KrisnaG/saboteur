...
from src.program.common_behaviour import find_goal_card_aim, euclidean_distance, infer_gold, \
    sabotage_player, mend_player, map_for_gold, evaluate_state
...
def find_best_path_card_placement(game_state, legal_actions, player, revealed):
    ...
    for path_action in path_actions:
        ...
        if path_action.find('dead') > 0:
            ...
            payoff = evaluate_bad_state(future_state['game-board'], aim, path_action)
...
def saboteur_behaviour(game_state, kb):
    ...
    if dynamite_exists:
        action = dynamite_path(legal_actions, game_state['game-board'],
    ...
    if sabotage_exists:
        action = sabotage_player(legal_actions, turn, kb, 'gold-miner', 0.9)
        if action is not None:
            return action
    ...
    if mend_exists and len(player['sabotaged']) > 0:
        action = mend_player(legal_actions, turn, player['sabotaged'], kb, 'saboteur')
        if action is not None:
            return action
