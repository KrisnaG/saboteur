...
class GoldMinerBehaviour(CommonBehaviour):
    ...
    def find_best_path_card_placement(game_state, legal_actions, player, revealed, kb):
        ...
        aim = GoldMinerBehaviour.find_goal_card_aim(player['seen'], revealed, game_state['announcements'], kb)
        ...
        for path_action in path_actions:
            ...
            payoff = GoldMinerBehaviour.evaluate_state(future_state['game-board'], aim, path_action)
    ...
    def dynamite_blocked_path(legal_actions, game_board, aim):
        ...
        for dynamite_action in dynamite_actions:
            ...
            if card.get_path_type().startswith('dead-end'):
                new_distance = GoldMinerBehaviour.euclidean_distance(x, aim[0], y, aim[1])
    ...
    def behaviour(game_state, kb):
        ...
        can_infer_gold = len(GoldMinerBehaviour.infer_gold(seen, revealed_cards)) == 1
        if map_exists and not gold_seen and not can_infer_gold:
            return GoldMinerBehaviour.map_for_gold(legal_actions, seen, revealed_cards)

        # Path
        path_exists = any(
            (("path" in action or "turn" in action) and action.find('dead') < 0 and action.find('pass') < 0)
            for action in legal_actions)
        if path_exists:
            return GoldMinerBehaviour.find_best_path_card_placement(
        ...
        if mend_exists and len(player['sabotaged']) > 0:
            action = GoldMinerBehaviour.mend_player(legal_actions, turn, player['sabotaged'], kb, 'gold-miner')
        ...
        if dynamite_exists:
            action = GoldMinerBehaviour.dynamite_blocked_path(
                legal_actions, game_state['game-board'],
                GoldMinerBehaviour.find_goal_card_aim(seen, game_state['revealed'], game_state['announcements'], kb))
            if action is not None:
                return action
        ...
        if sabotage_exists:
            action = GoldMinerBehaviour.sabotage_player(legal_actions, turn, kb, 'saboteur', 0.6)
            if action is not None:
                return action

        # Pass
        return [GoldMinerBehaviour.choose_card_to_discard(legal_actions, gold_seen)]
